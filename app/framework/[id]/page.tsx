  ...
  if (!params?.id) {
    return (
      <div className="min-h-screen bg-[#141414] p-6 flex items-center justify-center">
        <div className="text-white text-xl">Framework not found</div>
      </div>
    );
  }

  const frameworkName = decodeURIComponent(params.id as string).replace(/-/g, ' ');
  const displayName = formatFrameworkName(frameworkName);
      ...
      <div className="min-h-screen bg-[#141414] p-6">
        <div>
          ...
          <div className="text-center text-gray-400 mt-20">
            ...
            <p className="text-lg">No websites are currently built with &ldquo;{displayName}&rdquo;.</p>
          </div>
        </div>
      </div>
    );
